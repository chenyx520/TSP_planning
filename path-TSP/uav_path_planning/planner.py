...
from .tsp_solver import nearest_neighbor_order, two_opt, orientation_dp
...
class UAVPathPlanning:
    ...
    def solve_global_optimization(self, start_pos):
        ...
        if n > 8:
            ...
            print("TSP solved. Constructing final path...")
        else:
            ...
            for order in permutations(range(n)):
                cost = 0
                cost += get_dist(start_pos, patches_data[order[0]]['start'])
                for k in range(n-1):
                    i = order[k]
                    j = order[k+1]
                    cost += get_dist(patches_data[i]['end'], patches_data[j]['start']) + patches_data[j]['len']
                cost += get_dist(patches_data[order[-1]]['end'], start_pos)
                if cost < min_total_cost:
                    min_total_cost = cost
                    min_order = list(order)
            best_path_indices = [(idx, 0) for idx in min_order]
        print("TSP solved. Constructing final path...")
        full_path = []
        curr_pos = start_pos
