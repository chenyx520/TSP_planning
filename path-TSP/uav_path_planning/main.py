...
class UAVPathPlanning:
    ...
    def build_visibility_graph(self, points_of_interest):
        ...
        margin = self.uav_width/2 + 2.0
        inflated_obstacles = [obs.buffer(margin, resolution=2).simplify(2.0, preserve_topology=True) for obs in self.obstacles]
    ...
    def find_path_avoiding_obstacles(self, start, end):
        ...
        G = self.build_visibility_graph([start, end])
        
        try:
            path_nodes = nx.shortest_path(G, source=start, target=end, weight='weight')
            return path_nodes
        except nx.NetworkXNoPath:
            return [start, end] # Fallback direct line (should not happen if reachable)
        ...
        if n > 11:
            try:
                ...
                def route_length(order):
                    s = sum(get_dist(centroids[order[k]], centroids[order[k+1]]) for k in range(len(order)-1))
                    s += get_dist(start_pos, centroids[order[0]])
                    s += get_dist(centroids[order[-1]], start_pos)
                    return s
                unvisited = set(range(n))
                order = []
                cur = min(unvisited, key=lambda i: get_dist(start_pos, centroids[i]))
                order.append(cur)
                unvisited.remove(cur)
                while unvisited:
                    nxt = min(unvisited, key=lambda i: get_dist(centroids[cur], centroids[i]))
                    order.append(nxt)
                ...
                best_path_indices = [(order[i], dirs[i]) for i in range(n)]
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    # 先计算一个粗略的路径，以便在路径上放置障碍物
    # 简单起见，我们在随机对的杂草块中间放置障碍物
    patch_centroids = [p.centroid for p in patches]
    ...
    for i in range(num_obstacles):
        ...
        while attempts < 100:
            ...
            # 随机选择两个杂草块（或起点）
            idx1 = random.randint(-1, len(patches) - 1)
            idx2 = random.randint(-1, len(patches) - 1)
            
            if idx1 == idx2: continue
            
            p1 = (0,0) if idx1 == -1 else (patches[idx1].centroid.x, patches[idx1].centroid.y)
            p2 = (0,0) if idx2 == -1 else (patches[idx2].centroid.x, patches[idx2].centroid.y)
